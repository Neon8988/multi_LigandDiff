...
def remove_mean_with_mask(x, batch_seg):
    N = torch.bincount(batch_seg).float().to(x.device)
    mean=scatter_add(x, batch_seg, dim=0)/N.view(-1,1)
    x = x - mean[batch_seg]
    return x
...
def remove_partial_mean_with_mask(x, center_of_mass_mask,batch_seg):
    """
    Subtract center of mass of ligands from coordinates of all atoms
    """
    x_masked = x * center_of_mass_mask
    mean=scatter_add(x_masked,batch_seg,dim=0)/scatter_add(center_of_mass_mask, batch_seg, dim=0).view(-1,1)
    x = x - mean[batch_seg]
    return x

def remove_partial_mean(x, center_of_mass_mask,batch_seg):
    """
    Subtract center of mass of ligands from coordinates of all atoms
    """
    x_masked = x * center_of_mass_mask
...
def recover_mean_with_mask(x, mean, batch_seg):
    """only recover the mean of the masked atoms"""
    #return x*(1-mask)+(x*mask+mean[batch_seg])*mask
